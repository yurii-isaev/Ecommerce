...
namespace WebAPI.Authentication.UseCases.Requests
{
  ...
  public class RegisterUserCommandHandler : IRequestHandler<RegisterUserCommand, ResponseModel>
  {
    ...
    public async Task<ResponseModel> Handle(RegisterUserCommand request, CancellationToken token)
    {
      try
      {
        ...
        if (result.Succeeded)
        {
          ...
          return await Task.FromResult(new ResponseModel(ResponseCode.Ok, MessageTypes.RegistrationSuccess, ""));
        }

        return await Task.FromResult(new ResponseModel(ResponseCode.Ok, MessageTypes.RegistrationFailed,
